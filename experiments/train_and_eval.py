...
def good_agent_action(agent,world):
    ...
    other_pos = []
    for other in world.agents: # 他エージェントの位置を取得
    ...
    target_edge = world_edge[idx] # 最もpredatorから遠い角を目的地とする
    epsilon = 0.0
    ...
    mag = 1
    if vec[0] < 0:
        act[2] = min(abs(vec[0])*mag,agent.max_speed)
    else:
        act[1] = min(vec[0]*mag,agent.max_speed)
    if vec[1] < 0:
        act[4] = min(abs(vec[1])*mag,agent.max_speed)
    else:
        act[3] = min(vec[1]*mag,agent.max_speed)
...
def train(arglist):
    ...
    with U.single_threaded_session():
        ...
        while True: # 行動し続ける
            ...
            #[,right,left,up,down]
            '''
            if curriculum: # カリキュラム学習ありならpreyの動きをvictimなものにする
                ...
                if train_step % 15 == 0:
                    tgt_sel += 1
                    if tgt_sel > len(env.world.agents) - 1:
                        tgt_sel = 0
                env.world.agents[-1].max_speed = 1.0
                if episode_no > 5000 or arglist.load_model_number > 5000:
                    # Asama
                    env.world.agents[-1].max_speed = 0.001
                    curriculum = False
                action_n[-1] = (good_act)
            elif not arglist.learning_prey: # カリキュラム学習でないならpreyは普通に逃げる
                if train_step % prey_maintain_duration == 0:
                    good_act, _ = good_agent_action(env.world.agents[-1],env.world)
                action_n[-1] = (good_act)
            '''
            ...
            new_obs_n, rew_n, done_n, info_n = env.step(action_n) # 1ステップ進める(この中身をいじれば良い) agent.action(obs)の中身もいじる
            target_move(env)
            episode_step += 1
            done = all(done_n)
            ...
            if terminal and (len(episode_rewards) % 100 == 0):
                print("G", arglist.g_counter, " ", arglist.exp_name, " at episodes: {}, time: {}".format(
                    len(episode_rewards), round(time.time()-past_t, 3)))
                past_t = time.time()
            ...
            if terminal and (len(episode_rewards) % arglist.save_rate == 0):
                ...
                bench_step = 0
                agent_info = np.zeros(len(env.agents))
                ...
                while True:
                    ...
                    if bench_step > arglist.benchmark_iters:# and bdone:
                        ...
                        writing_info = np.append(writing_info, [np.var(episode_rewards[-arglist.save_rate:])])
                        writing_info = np.append(writing_info, [np.percentile(episode_rewards[-arglist.save_rate:],[0,25,50,75,100])])
                        writing_info = np.append(writing_info, round(time.time()-t_start, 3))
                        print('Finished benchmarking, at episode: ', episode_no, 'Score: ',agent_info[-1].astype(np.int64), '  in detail, ', agent_info[0:len(env.world.agents)+1])

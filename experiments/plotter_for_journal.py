...
#              ['CG_DDPG_org_5PMD', 'CG_DDPG_org_woCur_5PMD', 'NG_DDPG_5PMD', 'NG_MADDPG_5PMD_woCur_noEval']]
...
for TC in TCS: #各ベンチマークディレクトリ毎に
    if os.path.isdir(res_dir+TC): #ディレクトリがあることを確認、なければスキップ
        ...
        for count, f in enumerate(targetfiles):
            ...
            total_df.loc[TC, 'adv_sum_'+str(count)] = dft['adv_sum'].max()
            total_df.loc[TC, 'simul_'+str(count)] = dft['Mutual Collision'].max()
            #s = pd.Series([dft['adv_sum'].max(), dft['Mutual Collision'].max()], index=[str(count)+'_c', str(count)+'_s'], name=TC)
            #total_df = total_df.append(s)
        ...
        '''
        df1 = copy.deepcopy(all_means)
        df2 = copy.deepcopy(all_std)
        for l in df1.columns:
            df1 = df1.rename(columns={l:'mean '+l})
        for l in df2.columns:
            df2 = df2.rename(columns={l:'std '+l})
        output = pd.concat([df1, df2], axis=1)
        output.to_csv('./statictics/'+TC+'_all_means_std.csv')
        '''
        ...
        fsize = 14
        ax1.text(38000, -60, 'Mean rewards', fontsize=fsize)
        ax3.text(14000, -200, 'Stacked mean collision counts of each Predators', fontsize=fsize)
        ax4.text(22000, -17, 'Mean simultaneous collision counts', fontsize=fsize)
...
total_df.to_csv('statictics.csv')

...
class Scenario(BaseScenario):
    ...
    def make_world(self):
        ...
        world.dim_c = 3 # 命令の次元数
        world.target_sens = 0.0 # ???
        num_good_agents = 1 # preyの数(const)
        num_superleader = 0 # super_leaderの数
    ...
    def observation(self, agent, world): # あるエージェントが環境を観測する
        ...
        for other in world.agents: # 自分以外のエージェントについてそれぞれ以下を行う
            ...
            if agent.adversary: # I am adversary(predator) agent
                ...
                if dis < agent.obs_r or (agent.force_id >= 0 and agent.force_id == other.forced_id) or (agent.forced_id >= 0 and agent.forced_id == other.force_id): # I can see this other (regardless of Prey or Predator)
                ...
                other_vel.append(other.state.p_vel)
        if agent.forced_id >= 0: # リーダーからの強制力を観測

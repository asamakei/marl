...
class Scenario(BaseScenario):
    ...
    def make_world(self):
        ...
        num_superleader = 1 # superleaderの数
        num_leader = 2 # leaderの数
        num_follower = 2 # followerの数
        ...
        for i, agent in enumerate(world.agents): # 各エージェントについて設定する
            ...
            if i < num_follower: # Followers
                ...
                if i < num_follower - 1: agent.forced_id = 2
    ...
    def is_observed(self, target, agent): # ターゲットがエージェントに観測されているか
        delta_pos = target.state.p_pos - target.state.p_pos
        dist = np.sqrt(np.sum(np.square(delta_pos)))
        return True if dist < agent.obs_r else False
    ...
    def adversary_reward(self, agent, world): # predator側の報酬 preyとの接触数と外に出ている分の和
        ...
        # SuperLeaderの報酬を見直す 2023/1/13
        if agent.advsuperleader:
            for a in landmarks:
                for other in adversary_agents:
                    if other.forced_id == agent.force_id and self.is_observed(a,other):rew+=1 # 指示対象のLeaderがTargetを観測していたら報酬
        elif agent.advleader:
            for a in landmarks:
                for other in adversary_agents:
                    if other.forced_id == agent.force_id and self.is_collision(a,other):rew+=10 # 指示対象のFollowerがTargetを補足したら報酬
        elif agent.advfollower:
            for a in landmarks:
                if self.is_collision(a,agent):rew+=10 # 自身がTargetを捕捉したら報酬

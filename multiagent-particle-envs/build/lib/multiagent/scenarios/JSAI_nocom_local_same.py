...
class Scenario(BaseScenario):
    ...
    def make_world(self):
        ...
        for i, agent in enumerate(world.agents):
            ...
            if i < num_adversaries -1: # 0, 1, 2: adversary followers
                ...
                agent.adversary = True
                agent.good = False
                agent.silent = True # There is no speaker
            ...
            agent.accel = 4.0 #3.0 if agent.adversary else 4.0
            #agent.accel = 20.0 if agent.adversary else 25.0
            agent.max_speed = 1.3 #1.0 if agent.adversary else 1.3
        # add landmarks
    ...
    def adversary_reward(self, agent, world):
        ...
        for a in good_agents:
            for other in world.agents: #includiong Me
                if other is a: continue #both good
                if self.is_collision(a, other) :
                    count += 1
                    if other == agent: #myself
                        rew += 10
            if count == 4:
                rew += 50
